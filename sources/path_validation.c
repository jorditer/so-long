...
t_map   *validate_map(char *file)
{
    ...
    printf("Debug: Map size - width: %d, height: %d\n", map->width, map->height);
    read_map(map, file);
    printf("Debug: After reading map\n");
    if (map->grid)
        printf("Debug: First line of map: %s\n", map->grid[0]);
    check_walls(map);
    printf("Debug: Walls checked\n");
    check_components(map);
    printf("Debug: Components checked\n");
    check_path(map);
    printf("Debug: Path checked\n");
    return (map);
}